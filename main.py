...
def get_arg():
    ...
    cfg.add_argument('--dataset', default='scanobjectnn')
    cfg.add_argument('--data_path',default='./processed_h5_files1')
    cfg.add_argument('--device',default='cuda')
    cfg.add_argument('--fs_head', default='PCIA')
...
def getQueryLabel(y, k_way):
    ...
    # 获取支持集标签
    support_labels = y[:k_way*cfg.n_shot]
    
    # 创建映射字典，只使用每个类别第一次出现的位置
    label_mapping = {}
    for i, label in enumerate(support_labels):
        # 如果标签已经在映射字典中，跳过
        if label not in label_mapping:
            # 将原始标签映射到其第一次出现的位置所对应的类别索引
            # 例如：在5-way 5-shot中，位置0,5,10,15,20对应类别索引0,1,2,3,4
            label_mapping[label] = i // cfg.n_shot
    
    # 获取查询集的原始标签
    query_ori_label = y[k_way*cfg.n_shot:]
