...
from model.backbone.pointMamba import PointMambaBackbone
from model.backbone.pointMambaDGCNN import PointMambaDGCNN
from model.backbone.mv import MVModel
# from model.backbone.pointMAE import PointTransformer
# =============================
...
from model.fs_module.PCIA import PCIA
...
class fs_network(nn.Module):
    def __init__(self,k_way,n_shot,query,backbone='pointAndview',fs='PCIA'):
        super().__init__()
        self.k=k_way
        self.n=n_shot
        self.query=query
        ...
        if backbone == 'pointAndview':
            ...
            # self.pointbb.load_model_from_ckpt(point_backbone_ckpt)
            # self.pointbb.train()
    ...
    def get_fs_head(self,fs):
        if fs == 'crossAtt_mixmodel':
            ...
            return PCIA(self.k, self.n, self.query)
        else:
    ...
    def get_backbone(self,backbone):
        if backbone=='dgcnn':
            ...
            return PointMambaDGCNN(), MVModel()
        elif backbone == 'view':
            return MVModel()
        else:
